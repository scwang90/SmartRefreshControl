...
class UIDemoFooterController: UITableViewController {
    ...
    lazy var footer: UIRefreshFooter = {
        let footer = self.initRefreshFooter();
        self.colorAccent = footer.colorAccent
        self.colorPrimary = footer.colorPrimary
        return footer;
    }()
    
    func initRefreshFooter() -> UIRefreshFooter {
    ...
    func changeTheme(_ primary: UIColor, _ accent: UIColor) {
        self.footer.colorAccent = accent;
        self.footer.colorPrimary = primary;
...
//    }
    ...
    override func viewDidLoad() {
        ...
        self.footer.attach(self.tableView);
        self.footer.addTarget(self, action: #selector(onLoadMore), for: .valueChanged)
//        self.footer.beginLoadMore()
    ...
    @objc func onLoadMore() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
            let lastGroup = self.groups[self.groups.count - 1];
            let items = lastGroup.items;
            lastGroup.items = items + items;
            self.tableView.reloadData();
            if (lastGroup.items.count <= 16) {
                self.footer.finishLoadMore();
            } else {
                self.footer.finishLoadMoreWithNoMoreData();
            }
        }
    }
...
//        return super.tableView(tableView, heightForHeaderInSection: section);
    ...
    override func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        return section == self.groups.count - 1 ? 0.001: 15;
    }
